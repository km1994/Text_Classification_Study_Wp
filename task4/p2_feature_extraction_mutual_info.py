...
'''
    特征词先转成tfidf，然后用互信息选取6000个特征。但是筛选的速度很慢 begin
'''
from sklearn.feature_selection import mutual_info_classif
from sklearn.feature_selection import SelectKBest
def text_features_by_mutual_info(train_data,train_y,test_data,max_features=6000):
    ...
    # 将数据格式转化为 list 形式
    new_train_x = list(map(list, new_train_x.toarray()))
    new_test_x = list(map(list, new_test_x.toarray()))
    return new_train_x, train_y, new_test_x
'''
    特征词先转成tfidf，然后用互信息选取6000个特征。但是筛选的速度很慢 end
'''
...
if __name__ == "__main__":
    ...
    # 数据写入
    train_feature_filename = "../resource/THUCNews_ch/t2_text_feature_mutual_info_cnews.train.txt"
    write_file(train_feature_filename, train_feature_list, train_labels, is_num=True)

    test_feature_filename = "../resource/THUCNews_ch/t2_text_feature_mutual_info_cnews.test.txt"
    write_file(test_feature_filename, test_feature_list, test_labels, is_num=True)

    del train_file_path,train_list, train_labels,test_file_path,test_list, test_labels,deleteN,
    del  test_feature_filename, train_feature_filename,train_feature_list, test_feature_list
